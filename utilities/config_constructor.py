...
class Config(object):
    ...
    def test_params(self):
        ...
        assert isinstance(self.device_ids, list), f'Inspect: {self.device_ids}'
        assert self.dout_p <= 1 and self.dout_p >= 0, f'drop out probability should be in [0, 1]'
        assert self.N > 0, f'N should be > 0 but provided: {self.N}'
        assert self.smoothing <= 1 and self.smoothing >= 0, f'smoothing coeff should be in [0, 1]'
        assert isinstance(self.reference_paths, list) and len(self.reference_paths) == 2
        assert isinstance(self.tIoUs, list) and len(self.tIoUs) > 0, f'tIoUs: {self.tIoUs}'
        assert self.tIoUs == [0.3, 0.5, 0.7, 0.9]

        if self.one_by_one_starts_at >= self.early_stop_after:
            print('IT WILL NOT GOING TO BE TESTED ON LEARNED PROPS')
        if 'video' in self.modality:
            assert self.video_features_path is not None
            assert self.video_feature_name is not None
            assert os.path.exists(
                self.video_features_path), f'doesnt exists: {self.video_features_path}'
            assert self.d_model_video > 1 and self.d_vid > 1, f'd_model_video and d_dim should be > 1'
            if self.use_linear_embedder is False:
                assert self.d_model_video == self.d_vid, 'self.d_model != self.d_vid'
            if self.video_feature_name == 'i3d':
                assert self.d_vid == 1024, 'self.d_vid != 1024 but I3D is used'
            assert self.d_ff_video is not None

        if 'audio' in self.modality:
            assert self.audio_features_path is not None
            assert self.audio_feature_name is not None
            assert os.path.exists(self.audio_features_path), f'doesnt exists: {self.audio_features_path}'
            assert self.d_model_audio > 1 and self.d_aud > 1, f'd_model_audio and d_dim should be > 1'
            if self.use_linear_embedder is False:
                assert self.d_model_audio == self.d_aud, 'self.d_model != self.d_vid'
            if self.audio_feature_name == 'vggish':
                assert self.d_aud == 128
            assert self.d_ff_audio is not None
